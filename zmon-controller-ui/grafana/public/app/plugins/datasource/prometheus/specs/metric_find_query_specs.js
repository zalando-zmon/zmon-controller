System.register(['test/lib/common', 'test/specs/helpers', '../datasource', '../metric_find_query'], function(exports_1) {
    var common_1, helpers_1, datasource_1, metric_find_query_1;
    return {
        setters:[
            function (common_1_1) {
                common_1 = common_1_1;
            ...
            common_1.describe('PrometheusMetricFindQuery', function () {
                ...
                common_1.describe('When performing metricFindQuery', function () {
                    ...
                    common_1.it('label_values(resource) should generate label search query', function () {
                        ...
                        var pm = new metric_find_query_1.default(ctx.ds, 'label_values(resource)');
                    ...
                    common_1.it('label_values(metric, resource) should generate series query', function () {
                        ...
                        var pm = new metric_find_query_1.default(ctx.ds, 'label_values(metric, resource)');
                        pm.process().then(function (data) { results = data; });
                        ctx.$httpBackend.flush();
                        ctx.$rootScope.$apply();
                        common_1.expect(results.length).to.be(3);
                    ...
                    common_1.it('metrics(metric.*) should generate metric name query', function () {
                        ...
                        var pm = new metric_find_query_1.default(ctx.ds, 'metrics(metric.*)');
                        pm.process().then(function (data) { results = data; });
                        ctx.$httpBackend.flush();
                        ctx.$rootScope.$apply();
                        common_1.expect(results.length).to.be(3);
                    ...
                    common_1.it('query_result(metric) should generate metric name query', function () {
                        ...
                        var pm = new metric_find_query_1.default(ctx.ds, 'query_result(metric)');
                        pm.process().then(function (data) { results = data; });
                        ctx.$httpBackend.flush();
                        ctx.$rootScope.$apply();
