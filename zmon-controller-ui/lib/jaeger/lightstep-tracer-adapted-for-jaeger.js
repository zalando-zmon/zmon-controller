	...
	var TransportBrowser = function () {
	    ...
	    _createClass(TransportBrowser, [{
	        ...
	        value: function _reportAJAX(auth, report, done) {
				var collectorEndpoint = '/api/v0/reports';

				var jaegerCollectorEndpoint = '/api/v2/spans';
				var jaegerReport = report.span_records.map(function (span) {
					return {
						traceId: span.trace_guid,
						name: span.span_name,
						id: span.span_guid,
						kind: 'CLIENT',
						localEndpoint: {
							serviceName: report.runtime.group_name
						},
						timestamp: span.oldest_micros,
						duration: span.youngest_micros - span.oldest_micros,
						debug: true,
						tags: span.attributes.reduce(function (obj, tag) {
	            ...
	            var url = protocol + '://' + this._host + ':' + this._port + collectorEndpoint;
