    ...
    function($http, $q, $log, APP_CONST, PreconditionsService) {
        ...
        var doHttpCall = function(httpMethod, endpoint, payload, extraHeaders, timeout, postSuccessProcessing) {
           ...
           // Opentracing headers ingestion
            if (window.opentracing && window.eumTracingConfig && !window.eumTracingConfig.config.xhr_instrumentation) {
        ...
        service.getAlertStats = function(alertId) {
            return doHttpCall("GET", `/api/v1/false-positive-rates/${alertId}`)
        };
