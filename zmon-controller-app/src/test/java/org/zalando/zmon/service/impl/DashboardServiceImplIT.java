...
public class DashboardServiceImplIT {
    ...
    private final DataGenerator<DashboardImport> dashboardGenerator = new DashboardGenerator();

    @Test
    public void testCreateDashboard() throws Exception {
        final DashboardImport dashboard = service.createOrUpdateDashboard(dashboardGenerator.generate());
        final List<DashboardImport> allDashboards = service.getAllDashboards();
    ...
    public void testUpdateDashboard() throws Exception {
        final DashboardImport genDashboard = dashboardGenerator.generate();

        // create first dashboard
        final DashboardImport dashboard0 = service.createOrUpdateDashboard(genDashboard);

        // change the name of the new dashboard
        genDashboard.setName(genDashboard.getName() + " NEW");

        // and create the second dashboard
        DashboardImport dashboard1 = service.createOrUpdateDashboard(genDashboard);
    ...
    public void testGetAllDashboards() throws Exception {
        ...
        // and create the second dashboard
        final DashboardImport dashboard1 = service.createOrUpdateDashboard(genDashboard);
