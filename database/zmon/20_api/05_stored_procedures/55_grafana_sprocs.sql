...
CREATE OR REPLACE FUNCTION get_grafana_dashboards(IN s_title TEXT, OUT id TEXT, OUT title TEXT, OUT dashboard TEXT, OUT "user" TEXT, OUT "tags" TEXT) RETURNS SETOF record AS
$$
...
$$
select tag_name, count(1) AS "count" from (select json_array_elements_text((gd_dashboard->'tags')::json) AS tag_name from zzm_data.grafana_dashboard) t group by 1;
$$ LANGUAGE SQL VOLATILE SECURITY DEFINER;