...
public class DefaultZMonPermissionService {
    ...
    public void verifyDeleteCommentPermission(final int commentId) {
        final AlertCommentRecord comment = alertDefinitionSProc.getAlertCommentRecordById(commentId);
    ...
    public void verifyEditDashboardPermission(final DashboardRecord dashboard) {
        Preconditions.checkNotNull(dashboard, "dashboard");

        boolean isAllowed = hasAddDashboardPermission();
        if (isAllowed && dashboard.getId() != null) {
            final List<DashboardRecord> dashboards = dashboardSProc.getDashboardRecords(Collections.singletonList(
                    dashboard.getId()));
